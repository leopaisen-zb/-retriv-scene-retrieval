...
实验输入为 `RetrInput_hh/` 目录下按文件路径统计的 99 张图片。该数据集已由人工按场景分入不同子目录，因此本实验将子目录名作为 ground truth 标签。

需要注意：当前实验未做全局内容去重，检索单元是“图片文件路径”。如果按 SHA256 文件内容哈希做全局去重，99 个文件路径对应 79 个唯一图片内容；其中存在同一图片同时出现在不同人工类别目录中的情况。因此，本报告中的召回结果应理解为“基于 99 个文件条目的检索实验”，而不是“基于 79 张去重图片内容的检索实验”。
...
1. 对每个 query，每个方法都对 99 个图片文件条目排序。
2. 分别取排序结果前 `10% / 20% / 30% / 50%` 作为候选集合。
3. 99 个文件条目对应候选数为 `10 / 20 / 30 / 50`。
...
- 数据规模较小，当前按文件路径统计为 99 个图片条目，按内容哈希全局去重后为 79 个唯一图片内容。
- 当前实验未做全局内容去重，因此同一图片如果存在于不同子目录，会作为不同文件条目参与检索和 GT 统计。
